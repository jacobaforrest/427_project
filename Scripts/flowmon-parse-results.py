...
class Flow(object):
    ...
    def __init__(self, flow_el):
        ...
        tx_duration = (float(flow_el.get('timeLastTxPacket')[0:-2]) - float(flow_el.get('timeFirstTxPacket')[0:-2]))*1e-9
        rx_duration = (float(flow_el.get('timeLastRxPacket')[0:-2]) - float(flow_el.get('timeFirstRxPacket')[0:-2]))*1e-9
        ...
        if rxPackets:
            ...
            self.hopCount = -1000
        if rxPackets:
            self.delayMean = float(flow_el.get('delaySum')[0:-2]) / rxPackets * 1e-9
